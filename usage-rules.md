...
- **`handle_slot_origin/2`** (optional) — receives the slot's typed consistent-point **origin**
  ...
  false` → the `CREATE_REPLICATION_SLOT` consistent_point (new slot); `reused?: true` → the slot's
  live `confirmed_flush_lsn` (a resumed slot; advances across reconnects as the stream is acked).
  ...
  LSN, instead of silently skipping WAL. A sink without the callback is unaffected (no extra query).
