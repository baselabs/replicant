defmodule Replicant.SnapshotterTest do
  ...
  describe "build_change/4 (spec §6.2)" do
    test "builds a string-keyed %Change{op: :snapshot} with nil commit_lsn and empty columns" do
      change = Snapshotter.build_change("public", "orders", ["id", "note"], [1, "hi"])
    ...
    test "carries a nil column value through as a real NULL (not dropped)" do
      change = Snapshotter.build_change("public", "orders", ["id", "note"], [1, nil])
