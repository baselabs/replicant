...
|---|---|---|---|---|
| B1 | **`replicant-snapshot-incremental`** — resumable / incremental snapshot (DBLog-derived chunking adapted to READ-ONLY sources, interleaved with the stream) | Large-table onboarding: today a crash at 99% of a multi-hour COPY redoes everything (all-or-nothing per attempt, exported snapshot dies with its session). The biggest adoption blocker. | snapshot spec §3 | 📋 **PLAN READY 2026-07-09** (plan-autopilot: 13 tasks, 8 opus/5 sonnet; machine-gated 0/0; independent review 10 findings/10 fixed incl. 1 blocking canonical-PK drop-set fix; P7 keepalive-ordering probe run live = SOUND; baseline 388/0 @ 7eb49dc). [design](superpowers/specs/2026-07-09-replicant-snapshot-incremental-design.md) · [plan](superpowers/plans/2026-07-09-replicant-snapshot-incremental.md) · next: /exec-autopilot |
