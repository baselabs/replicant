...
**Updated:** 2026-07-09 (backlog audit) · **Core HEAD:** `499c8ba` (docs-only past the published `6fe9d3c` = `v0.1.0`) · **Branch:** `main`
...
| A5 | **PG17+ forward-compat + failover slots** | Invalidation detection is PG16-keyed (`wal_status`/`conflicting`); PG17 adds `invalidation_reason`. PG17 `failover = true` slots → HA resume-on-promoted-standby. CI matrix is PG16-only. | [query_builder.ex:123-127](../lib/replicant/query_builder.ex) (comments acknowledge the PG17 divergence) | Open |

### B. Spec-named future slices (owners assigned in shipped specs)

| # | Item | Unlocks | Named in | Status |
|---|---|---|---|---|
| B1 | **`replicant-snapshot-incremental`** — resumable / incremental snapshot (DBLog-derived chunking adapted to READ-ONLY sources, interleaved with the stream) | Large-table onboarding: today a crash at 99% of a multi-hour COPY redoes everything (all-or-nothing per attempt, exported snapshot dies with its session). The biggest adoption blocker. | snapshot spec §3 | 🏗️ **IN BUILD 2026-07-09** (exec-autopilot started; plan-autopilot: 13 tasks, 8 opus/5 sonnet; machine-gated 0/0; independent review 10 findings/10 fixed incl. 1 blocking canonical-PK drop-set fix; P7 keepalive-ordering probe run live = SOUND; baseline 388/0 @ 7eb49dc). [design](superpowers/specs/2026-07-09-replicant-snapshot-incremental-design.md) · [plan](superpowers/plans/2026-07-09-replicant-snapshot-incremental.md) · next: exec then /review-autopilot |
...
| # | Item | Unlocks | Status |
|---|---|---|---|
| C1 | `Replicant.Telemetry.metrics/0` (`Telemetry.Metrics` definitions) | Oban/Broadway convention — 27 structure-only events exist; every consumer currently hand-authors metric definitions | Open |
