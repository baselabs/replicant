...
|---|---|---|---|---|
| B1 | **`replicant-snapshot-incremental`** — resumable / incremental snapshot (DBLog-derived chunking adapted to READ-ONLY sources, interleaved with the stream) | Large-table onboarding: today a crash at 99% of a multi-hour COPY redoes everything (all-or-nothing per attempt, exported snapshot dies with its session). The biggest adoption blocker. | snapshot spec §3 | 📐 **Spec APPROVED 2026-07-09** (brainstorm-autopilot: live-PG16 Stage-0 probes; Stage-6 fresh-context review 8 challenges / 2 blocking, all folded). [design](superpowers/specs/2026-07-09-replicant-snapshot-incremental-design.md) · next: plan-autopilot |
